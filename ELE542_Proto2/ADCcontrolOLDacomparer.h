...
volatile float ADCcompile_value; 
...
void compileAdcData(void)
{
	...
	if ((MOTOR_DIR_IN_LEFT & PINA) == MOTOR_DIR_IN_LEFT)
	{
		...
		if (adcValues.left < leftLimits.negZero)  // Comparaison avec valeur sur 1, NO!
		{
	...
	else
	{
		if (adcValues.left > leftLimits.posZero)
		{
			...
			adcValues.left = adcValues.left / leftLimits.adcMaxValueHigh; //adcValues sur 1
		}
		else
		{
			adcValues.left = 0.0;
...
void calibrateMotors(void)
{
	...
	for (i=0;i<CAL_SAMPLES;i++){
		...
		compileAdcData();
		//waitValueIsReady();
		ADCcompile_value += adcValues.left * MAX_ADC_VALUE;
	}
	leftLimits.posMax = ADCcompile_value / CAL_SAMPLES;
	ADCcompile_value = 0;
	...
	for (i=0;i<CAL_SAMPLES;i++){
		...
		compileAdcData();
		//waitValueIsReady();
		leftLimits.posZero += adcValues.left * MAX_ADC_VALUE;
	}
	leftLimits.posZero = ADCcompile_value / CAL_SAMPLES;
	ADCcompile_value = 0;	
	...
	for (i=0;i<CAL_SAMPLES;i++){
		...
		compileAdcData();
		//waitValueIsReady();
		leftLimits.negMax += adcValues.left * MAX_ADC_VALUE;
	}
	leftLimits.negMax = ADCcompile_value / CAL_SAMPLES;
	ADCcompile_value = 0;
	...
	for (i=0;i<CAL_SAMPLES;i++){
		...
		compileAdcData();
		//waitValueIsReady();
		leftLimits.negZero += adcValues.left * MAX_ADC_VALUE;
	}
	leftLimits.negZero = ADCcompile_value / CAL_SAMPLES;
	ADCcompile_value = 0;

	// Remove left motor deadzone
	...
	for (i=0;i<CAL_SAMPLES;i++){
		...
		compileAdcData();
		//waitValueIsReady();
		rightLimits.posMax = adcValues.right * MAX_ADC_VALUE;
	}
	rightLimits.posMax = ADCcompile_value / CAL_SAMPLES;
	ADCcompile_value = 0;
	...
	for (i=0;i<CAL_SAMPLES;i++){
		...
		compileAdcData();
		//waitValueIsReady();
		rightLimits.posZero = adcValues.right * MAX_ADC_VALUE;
	}
	rightLimits.posZero = ADCcompile_value / CAL_SAMPLES;
	ADCcompile_value = 0;
	...
	for (i=0;i<CAL_SAMPLES;i++){
		...
		compileAdcData();
		//waitValueIsReady();
		rightLimits.negMax = adcValues.right * MAX_ADC_VALUE;
	}
	rightLimits.negMax = ADCcompile_value / CAL_SAMPLES;
	ADCcompile_value = 0;
	...
	for (i=0;i<CAL_SAMPLES;i++){
		...
		compileAdcData();
		//waitValueIsReady();
		rightLimits.negZero = adcValues.right * MAX_ADC_VALUE;	
	}
	rightLimits.negZero = 	ADCcompile_value / CAL_SAMPLES;
	ADCcompile_value = 0;

	// Remove right motor deadzone
